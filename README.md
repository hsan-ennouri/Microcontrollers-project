...
Custom Peripheral Drivers: Coded bare-metal drivers from scratch to interface with a 4x4 matrix keypad and an alphanumeric LCD display.
...
## Hardware Components
ATmega Microcontroller (e.g., ATmega328P / ATmega16)
...
Architected the system around Interrupt Service Routines (ISRs) and Hardware Timers. The main execution loop remains lightweight, while time-critical tasks such as debouncing the keypad, toggling the buzzer at specific frequencies, and stepping the motor are handled via precise timer overflows and external interrupt triggers. This ensures zero latency between keystrokes and audio-visual feedback.
